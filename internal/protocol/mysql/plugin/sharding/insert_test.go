...
func TestShardingInsert_Build(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			wantQs: []sharding.Query{
				{
					SQL:        fmt.Sprintf("INSERT INTO %s.%s(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);", "`order_db_1`", "`order_tab_1`"),
					Args:       []any{1, 3, "content", 1.1},
					DB:         "order_db_1",
		...
		},
		{
			...
			wantQs: []sharding.Query{
				{
					SQL:        fmt.Sprintf("INSERT INTO %s.%s(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);", "`order_db_0`", "`order_tab_1`"),
					Args:       []any{4, 4, "4", 4.0},
					DB:         "order_db_0",
					Datasource: "0.db.cluster.company.com:3306",
				},
				{
					SQL:        fmt.Sprintf("INSERT INTO %s.%s(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);", "`order_db_0`", "`order_tab_2`"),
					Args:       []any{2, 2, "2", 2.0},
					DB:         "order_db_0",
					Datasource: "0.db.cluster.company.com:3306",
				},
				{
					SQL:        fmt.Sprintf("INSERT INTO %s.%s(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);", "`order_db_1`", "`order_tab_0`"),
					Args:       []any{3, 3, "3", 3.0},
					DB:         "order_db_1",
					Datasource: "1.db.cluster.company.com:3306",
				},
...
func (s *ShardingInsertSuite) TestShardingInsert_Exec() {
	...
	testcases := []struct {
		...
		},
		{
			...
			mockDb: func() {
				s.mock02.MatchExpectationsInOrder(false)
				s.mock02.ExpectExec(regexp.QuoteMeta("INSERT INTO `order_db_1`.`order_tab_1`(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);")).WithArgs(1, 1, "1", 1.0).WillReturnError(newMockErr("db01"))
				s.mock02.ExpectExec(regexp.QuoteMeta("INSERT INTO `order_db_1`.`order_tab_0`(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);")).WithArgs(3, 3, "3", 3.0).WillReturnResult(sqlmock.NewResult(1, 1))
				s.mock01.ExpectExec(regexp.QuoteMeta("INSERT INTO `order_db_0`.`order_tab_2`(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);")).WithArgs(2, 2, "2", 2.0).WillReturnResult(sqlmock.NewResult(1, 1))
			},
		...
		},
		{
			...
			mockDb: func() {
				s.mock02.MatchExpectationsInOrder(false)
				s.mock02.ExpectExec(regexp.QuoteMeta("INSERT INTO `order_db_1`.`order_tab_1`(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);")).WithArgs(1, 1, "1", 1.0).WillReturnError(newMockErr("db"))
				s.mock02.ExpectExec(regexp.QuoteMeta("INSERT INTO `order_db_1`.`order_tab_0`(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);")).WithArgs(3, 3, "3", 3.0).WillReturnError(newMockErr("db"))
				s.mock01.ExpectExec(regexp.QuoteMeta("INSERT INTO `order_db_0`.`order_tab_2`(`uid`,`order_id`,`content`,`account`) VALUES(?,?,?,?);")).WithArgs(2, 2, "2", 2.0).WillReturnError(newMockErr("db"))
			},
