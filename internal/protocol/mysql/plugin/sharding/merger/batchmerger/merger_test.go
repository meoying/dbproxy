...
import (
	...
	"github.com/meoying/dbproxy/internal/protocol/mysql/plugin/sharding/merger/errs"
	"testing"
	...
	testcases := []struct {
		...
		},
		{
			...
			rowsList: func() []sqlx.Rows {
				...
				rowsList := make([]sqlx.Rows, 0, len(dbs))
				for _, db := range dbs {
